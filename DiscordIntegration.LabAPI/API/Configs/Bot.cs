    ...
    public sealed class Bot
    {
        ...
        internal static async Task UpdateActivity(CancellationToken cancellationToken)
        {
            while (true)
            {
                try
                {
                    Logger.Debug($"{nameof(UpdateActivity)}: Updating bot activity: {Player.Count}/{Instance.Slots}", Instance.Config.Debug);
        ...
        internal static async Task UpdateChannelsTopic(CancellationToken cancellationToken)
        {
            while (true)
            {
                ...
                try
                {
                    ...
                    await Network.SendAsync(new RemoteCommand(ActionType.UpdateChannelActivity, $"{string.Format(Language.PlayersOnline, Player.Dictionary.Count, Instance.Slots)}. {string.Format(Language.RoundDuration, Round.Duration)}. {string.Format(Language.AliveHumans, aliveHumans)}. {string.Format(Language.AliveScps, aliveScps)}. {warheadText} IP: {Server.IpAddress}:{Server.Port} TPS: {Server.Tps}"), cancellationToken);
                }
                catch (Exception exception)
                {
                    Logger.Error(string.Format(Language.CouldNotUpdateChannelTopicError, exception));
                }
