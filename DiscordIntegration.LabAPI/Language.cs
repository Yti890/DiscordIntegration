...
namespace DiscordIntegration
{
    ...
    public sealed class Language
    {
        ...
        /// </summary>
        public static string Folder { get; } = Path.Combine(LabApi.Loader.Features.Paths.PathManager.Configs.ToString(), Instance.Name, "Languages");
...
#pragma warning restore SA1600 // Elements should be documented
        ...
        public void Load()
        {
            ...
            {
                Logger.Error($"Error! Failed to read {Instance.Config.Language} language, located at \"{FullPath}\": {exception}.\nDefault translation will be used.");
                return;
            }
            finally
            {
        ...
        {
            Logger.Debug($"[Language.Save] Path: {FullPath}, Overwrite: {shouldOverwrite}", Instance.Config.Debug);

            if (File.Exists(FullPath) && !shouldOverwrite)
            {
                Logger.Debug("[Language.Save] File exists and overwrite is false, skipping save.");
                return;
            }
            ...
            try
            {
                ...
                {
                    Logger.Debug("[Language.Save] Creating language folder...", Instance.Config.Debug);
                    Directory.CreateDirectory(Folder);
                }
            }
            catch (Exception exception)
            {
                Logger.Error($"Error! Failed to create language directory at \"{Folder}\": {exception}.");
                return;
            }

            try
            {
                Logger.Debug("[Language.Save] Serializing language to file...", Instance.Config.Debug);
                using (var streamWriter = new StreamWriter(FullPath))
                {
                    jsonSerializer.Serialize(streamWriter, this);
                }

                Logger.Info($"[Language.Save] Language file successfully saved to: {FullPath}");
            }
            catch (Exception exception)
            {
                Logger.Error($"Error! Failed to create \"{Instance.Config.Language}\" language at \"{FullPath}\": {exception}.");
            }
        }

        private void Error(object sender, Newtonsoft.Json.Serialization.ErrorEventArgs ev)
        {
            Logger.Warn($"Translation not found for \"{ev.ErrorContext.Member}\" key, loading default one...");
